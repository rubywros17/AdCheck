...
public class ProductContentExtractionService {
    ...
    private String buildPrompt(String cleanedText, Map<String, String> ocrResults, List<String> allLines) {
        ...
        sb.append("- 제외 대상: 배송안내, 보관방법, 브랜드/회사 소개, 이벤트 안내\n");
        // "단순 성분·원재료 나열"을 제외 대상으로만 적어뒀더니 "단순"의 경계를 모델이 매번 새로
        // 판단해서 결과가 흔들렸다(2026-09-22 실측: 같은 입력 4회에 Claim 3/5/3/3건, 안정도 60%).
        // 흔들린 문구가 둘 다 성분·함량 관련이었기에, 판단을 요구하는 대신 대조만 하면 되도록
        // 세 갈래로 명시한다. ClaimExtractionStabilityTest로 개선 여부를 잰다.
        sb.append("- 성분 관련 문장은 아래 기준에 대조해서 나누세요(\"단순한 나열인지\"를 스스로 판단하지 마세요):\n");
        sb.append("  · 원재료명 표시(표·목록 형태로 성분과 함량이 나열된 부분) → 제외. [항목 3]에서 따로 다룹니다. ");
        sb.append("예: \"원재료명: 밀크씨슬추출물 50%, 비타민B1 100%, 나이아신 20%\"\n");
        // 2026-09-23 실측(분석 45, exxxtreme.co.kr/product_no=69): 뽑힌 Claim 3건이 전부
        // 기능성 내용 표시란이었다. 표시란은 인정받은 문구를 그대로 옮긴 법정 표시사항이라
        // 과장 광고가 아닌데도 "성분의 기능·작용을 설명하는 문장 → 포함"에 걸려 올라왔다.
        // 위 원재료명 표시와 같은 성격(광고 카피가 아니라 라벨)이므로 제외 갈래로 붙인다.
        // ClaimExtractionLabelSectionTest가 이 세 문구를 회귀 케이스로 고정한다.
        sb.append("  · 기능성 내용 표시란(원료명을 머리말·대괄호로 달고 인정받은 기능성 문구를 그대로 옮긴 줄, ");
        sb.append("또는 성분명과 함량만 적은 줄) → 제외. 광고 카피가 아니라 라벨 표기입니다. ");
        sb.append("예: \"[밀크씨슬추출물] 간 건강에 도움을 줄 수 있음\", \"[비오틴] 지방, 탄수화물, 단백질 대사와 에너지 생성에 필요\", \"실리마린 130mg\"\n");
        sb.append("  · 본문에서 특정 성분의 함량·비율·충족률을 내세우는 문장 → 포함. ");
        sb.append("예: \"1정당 130mg 함유, 주원료 기준 100% 충족합니다\"\n");
